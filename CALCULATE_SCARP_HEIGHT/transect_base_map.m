% This extracts the faults positions and performs the coordinate rotation
% to plot the faults along the topographic profile. 

average_height=[];
for j=1:.2:max(xc)
    f=find(xc>=j&xc<j+.3);
  average_height =[average_height;[j mean(zc(f))]];
end
